...
| 12 | **I. 友達バックエンド=Supabase。疎通検証済→解禁準備** | 🟢🟢 **疎通検証済み (2026-05-31)**: Supabaseプロジェクト作成+schema実行+Secrets記入 完了。**バックエンドREST疎通(2匿名ユーザーで申請→承認→双方向表示→チア+RLS負荷)全合格 / iOSアプリ実コード(supabase-swift)からも書込成功**(2台目Apple ID不要で実証)。フロントUIテスト5/5 pass・3LLM監査リーク2件是正済。**解禁の残作業**: ①テストデータ掃除(SQL)✅ ②監査指摘のUI改善 ✅**完了**(エラーバナー/初回ローディング/旧説明文刷新/cheer連打ガード/QRディープリンク化。設計=`docs/superpowers/specs/2026-05-31-friends-uiux-shipping-quality-design.md`、Codex監査ループで承認・ビルド成功・UIテスト5/5。※ユニットは当環境でApp Group署名hang=Xcodeで要実行) ③`friendsEnabled` Release=true ✅ ④コード側の解禁完了 ✅ **ブランチ `feature/friends-release` に実装済**(2026-05-31・Codex改善ループ4周で承認・ビルド成功): フラグ解禁/PrivacyInfo収集申告(Fitness+UserContent+UserID)/活動共有完成(今日のカテゴリ・種目名+詳細opt-in+月次)/Release Supabase固定/プライバシーポリシーをSupabase・活動共有に更新。⑤CloudKitFriendsService削除✅(Supabase移行で不要) ⑥v1.1メタデータ作成✅=`submission/app_store_metadata_v1.1.md`(友達入り・収集ありPrivacy・絵文字なし) ⑦テストhost hang恒久修正✅(友達27ユニットテスト緑) ⑧**友達UI/UX「最高級」化** ✅完了(2026-05-31)=6項目(shareカードスリム化/自分行ハイライト既存確認/チア絵文字バースト演出[reduceMotion尊重・id判定]/カード階層既存確認/空状態の待機猫イラスト/FriendAvatar識別リング[FNV-1a→hue])。FriendAvatarIdentityRingTests追加・友達ユニット15緑・Codex 2点修正→承認。設計=`docs/superpowers/specs/2026-05-31-friends-uiux-premium-design.md`。⑨**サインインの壁カード廃止→自動匿名サインイン化** ✅完了(2026-05-31)=友達タブ表示時に `ensureSignedIn()`(upsert冪等・isSigningIn再入ガード)で裏サインイン、壁を spinner/失敗時再試行に置換。初回のみ表示名インライン入力(既定名「ねこの友」・成功時のみdismiss)+ヘッダー鉛筆で表示名変更(`updateDisplayName`→publishMyProfile、username/friendCode不変)。`FriendsSignInSheet`削除。ユニット+4(25緑)・UIテスト書換・Codex 要修正3→承認・シミュレータ目視OK。設計=`docs/superpowers/specs/2026-05-31-friends-auto-signin-design.md`。⑩**友達アカウント堅牢化(匿名ファースト維持+lazy化+任意連携設計)** ✅Phase1完了(2026-06-01)=匿名サインインの3懸念(機種変で復旧不可/孤児アカウント蓄積/opt-in緊張)を払拭する設計。**lazy化**: タブ表示ではクラウド非アクセス、「友達とつながる」/deep link承認の能動操作でのみ匿名サインイン(`profile`生成経路をsignedInBodyの手前に限定。welcome画面新設・失敗はインライン+CTA再試行)。**運用ガード**: 孤児削除cron`supabase/cron/cleanup_orphans.sql`(friendship/申請ゼロ+30日無更新の匿名のみ削除・permanent除外・EXECUTE剥奪)+`profiles.updated_at`トリガ+CAPTCHA手順(クライアントtoken対応とセットで有効化と明記)。友達テスト緑(ユニット17+UI5・lazy検証に書換)・**Codex 3巡で"patch is correct"**。設計=`docs/superpowers/specs/2026-06-01-friends-account-hardening-design.md`。**CAPTCHAクライアントtoken送信** ✅実装完了(2026-06-01・config-gated)=`CaptchaTokenProviding`/`NoCaptchaTokenProvider`(no-op)/`TurnstileCaptchaTokenProvider`(WKWebView・キャンセル/タイムアウト/タスクキャンセル対応)。`SupabaseFriendsService.ensureUID`が新規匿名サインイン時のみ`signInAnonymously(captchaToken:)`へ。`TURNSTILE_SITE_KEY`(Secrets.xcconfig→Info.plist)空なら無効=従来挙動バイト互換。CaptchaTests追加(ユニット198緑)・Codex 3巡で"correct"。**有効化(残・キー所有者)**: Turnstileキー発行+許可ドメイン登録→Secrets設定→Supabase側CAPTCHA ON(同時)。※実webview経路は実機+実キーで手動確認。 **Phase1残**: cron本番実データ検証 / ASCダッシュボードへApp Privacyラベル反映(文面は下記✅)。**プライバシー/規約/メタデータ文面のlazy整合** ✅完了(2026-06-01)=`PrivacyPolicy.md`/`docs/privacy.md`(タブ表示では非保存・能動操作時のみ送信を明記/孤児30日自動削除を開示/§2共有項目を実装と一致[週月運動時間・週間達成・猫種類追加]/§1§3§4矛盾解消)・`TermsOfService.md`/`docs/terms.md`(誤記「Apple ID/iCloud必須」「iCloud/CloudKit障害」→Supabase匿名認証に是正)・`app_store_metadata_v1/v1.1`(CloudKit残骸是正・正本ポインタ修正)・旧`app_store_metadata.md`に非推奨バナー。Codex 4巡で"correct"(0.97)。**Phase2 = Apple連携(①復旧)** ✅実装完了(2026-06-01・config-gated・壁打ちCodexで設計確定)=匿名uidを保持したままApple連携で永続化。`AppleSignInCoordinator`(ASAuthorization+SHA256 nonce)→`linkIdentityWithIdToken(.apple)`。衝突(`identityAlreadyExists`)はマージせず「切替/中止」二択(切替=`signInWithIdToken`+profile再ロード、孤児はcron回収)。`backupStatus`/`isBackedUp`公開、消せるバックアップカード(friends>=1||streak>=7)。`FriendsAppleLinkEnabled`/`FriendsGoogleLinkEnabled`(Info.plist既定false)空なら非表示=現匿名挙動不変。AccountLinkingTests追加(ユニット205緑)。Codex 4巡で"correct"(0.94)。修正反映=switch後profileロード/再入ガード/nonce失敗throw/signOut連携済保持・TOCTOU解消/backupStatus signOut・signIn整合。**Phase2残(follow-up)**: 復元入口(新端末でApple復元。無いと新端末で新規匿名化) / Google web連携 / 設定(Apple capability・Supabase provider・Manual Linking ON)後に`Friends*LinkEnabled=true`・実機検証。設計=`docs/superpowers/specs/2026-06-01-friends-account-hardening-design.md`(Phase 2節)。 **Phase2旧メモ**: Apple/Google identity linking(任意・後追い促し・衝突はマージせず二択)で①根治。**残=ユーザー作業(v1.0承認後にv1.1として)**: App Privacyラベル更新(ASC) / 版+1・アーカイブ・アップロード / 友達スクショ再撮影 / **実機2台の実フロー検証**(推奨)→ ブランチをmainにマージ。BE設計=`docs/friends_backend_crossplatform.md` |
