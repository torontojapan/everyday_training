...
7. ★★**実スクショ無しに「✅/完了/一致/MATCH/パリティOK」と書くの絶対禁止**(2026-06-18 セッション2 再厳命)。
   コード照合・コンパイル成功・ユニットテスト green は**視覚の正しさを一切保証しない**。各画面で必ず
   `:app:assembleDebug`→`adb install -r`→該当画面へ遷移→`adb exec-out screencap -p`→Read で開いて拡大目視→
   **iOS build 12 golden と横並びで 高さ/色/余白/角丸/フォント/整列/不透明度 を1要素ずつ照合**してから報告。
   **同一行に異なるコンポーネント型を混ぜない**(Material field と自作 Box の混在=高さ不揃い)。**iOS の塗り・色トークン
   (chipBackground 等)を移植**し、Material 既定(OutlinedTextField 白枠)で代用しない。高さは明示的に揃える。詳細 [[feedback_parity_screenshot_mandatory]]。
- 失敗実例(2026-06-18 ①): 記録入力を「MATCH」誤報告(種目メモ欄欠落・重さ配置違い・候補が日本語デフォルト未表示・
  アコーディオン無し)。reviewer に「データ差は無視」と広く言い、iOS ソース未精読が原因。
- 失敗実例(2026-06-18 ②): セッション2で6画面をコード照合のみで「✅完了」と報告 → 実画面は記録入力の入力欄が
  高さ不揃い・カード/フィールド色が iOS と別物。**スクショ未確認で完了報告した**のが原因。既報告画面は全て未検証扱いで再照合。
