...
internal fun ProfileRow.toProfile(): FriendProfile = FriendProfile(
    ...
    // my_cat_breed には PetBreed.friendBreedString が載る(猫=素の raw "orange" / 犬="dog:shiba")。
    // fromStorage が両形式 + 旧形式(prefix 無し猫 raw)を解釈。null/空はそのまま null(hash 既定)。
    myPet = myCatBreed?.takeIf { it.isNotBlank() }?.let { com.goexercise.app.domain.PetBreed.fromStorage(it) },
...
class MockFriendsService : FriendsService {
    ...
    private fun seedDemo() {
        if (friends.isEmpty()) {
            friends.add(FriendProfile("ABC234", "haru", "はる", currentStreak = 12, totalAchievedDays = 40, todayAchieved = true, weeklyTotalMinutes = 120, weeklyAchievements = listOf(true, true, false, true, true, false, false), weeklyStatuses = listOf(com.goexercise.app.domain.DailyStatus.Achieved, com.goexercise.app.domain.DailyStatus.Rest, com.goexercise.app.domain.DailyStatus.Rescued, com.goexercise.app.domain.DailyStatus.Achieved, com.goexercise.app.domain.DailyStatus.TodayAchieved, com.goexercise.app.domain.DailyStatus.Future, com.goexercise.app.domain.DailyStatus.Future), myPet = com.goexercise.app.domain.PetBreed.Dog(com.goexercise.app.domain.DogBreed.Shiba), todayCategoryName = "筋トレ", todayExerciseDetails = listOf(com.goexercise.app.domain.friends.SharedExerciseDetail("スクワット", reps = 20, sets = 3), com.goexercise.app.domain.friends.SharedExerciseDetail("ベンチプレス", durationMinutes = 20, reps = 10, sets = 3)), lastUpdated = java.time.Instant.now().minusSeconds(7200), connectedSince = java.time.Instant.now().minusSeconds(30L * 86400)))
            friends.add(FriendProfile("XYZ789", "kenta", "けんた", currentStreak = 3, totalAchievedDays = 8, todayAchieved = false, weeklyTotalMinutes = 45, weeklyAchievements = listOf(true, false, false, true, false, false, false), weeklyStatuses = listOf(com.goexercise.app.domain.DailyStatus.Achieved, com.goexercise.app.domain.DailyStatus.Missed, com.goexercise.app.domain.DailyStatus.Rest, com.goexercise.app.domain.DailyStatus.Achieved, com.goexercise.app.domain.DailyStatus.TodayPending, com.goexercise.app.domain.DailyStatus.Future, com.goexercise.app.domain.DailyStatus.Future), myPet = com.goexercise.app.domain.PetBreed.Cat(com.goexercise.app.domain.CatBreed.Gray), lastUpdated = java.time.Instant.now().minusSeconds(86400), connectedSince = java.time.Instant.now().minusSeconds(7L * 86400)))
        }
        if (incoming.isEmpty()) {
