...
## 1.1 ホームのスター行
**配置**: `HomeView` 上部、`topStatusBar`(左=連続記録バッジ / 右=今日は達成済み)の**すぐ下に新しい行**を追加。

**表示ロジック(純関数・テスト可能)**: `ReferralStarsDisplay.style(count: Int) -> Style`
```
enum Style {
    case ghost                 // 0個: 薄い枠星 ☆ 1個
    case progress(filled: Int, total: 10)  // 1〜9個: 金星 filled + 枠星 (10-filled)
    case complete              // 10個: 金星 ×10(解放達成)
    case collapsed(Int)        // 11個以上: 「⭐ N」
}
```
- **0個**: 薄いグレー枠の星 ☆ を1個(「これから光る枠」)。控えめ。
- **1〜9個**: 金の星 ⭐ を count 個 + 残りを薄い枠星で**10個まで**並べる(進捗が一目で分かる=10を目指したくなる)。横幅に入らなければ2行目へ折り返し可。
- **10個**: ⭐×10 全点灯(解放状態)。
- **11個以上**: 「**⭐ N**」に省略(個別の星は出さない)。
- 補助テキスト(任意・小さく): `1 ≤ count < 10` のとき右に `あと{10-count}人で猫が解放`。10以上では出さない。
...
**0個時の扱い**: 行は常に存在(`AppFeatureFlags.isReferralActive` かつサインイン時)。星0でも ghost 星1個 + タップ招待で「枠を埋めたい」動機を作る。未サインイン/紹介無効時は行ごと非表示。
...
- **友達紹介**: 1人紹介ごとに ⭐ + フリーズ。**⭐10個で好きな猫が無料解放**
